    ...
    return state.root.children.map((node, i) => {
      ...
      if (node.type === 'list') {
        return (
          ...
          >
            {elementNode.children?.map((child, idx) => (
              <li key={idx}>{'children' in child ? renderChildren((child as SerializedElementNode).children) : null}</li>
            ))}
          </ul>
        )
      }
      ...
      if (node.type === 'paragraph') {
        return <p key={i}>{renderChildren(elementNode.children)}</p>
      }

      if (node.type === 'heading') {
        return <h3 key={i}>{renderChildren(elementNode.children)}</h3>
      }

      return null
    })
  }
  const hasRichTextContent = (state?: DefaultTypedEditorState) => {
    if (!state) return false


    const walk = (nodes?: SerializedLexicalNode[]): boolean => {
      if (!nodes) return false
      return nodes.some((node) => {
        if (node.type === 'text') {
          return Boolean((node as SerializedTextNode).text?.trim())
        }
        if ('children' in node) {
          return walk((node as SerializedElementNode).children)
        }
        return false
      })
    }

    return walk(state.root.children)
  }
  return (
    <section className="hero-section pt-[140px]">
          ...
          <div className="inner-content flex flex-col lg:flex-row lg:space-y-0 space-y-[56px]">
            <div className="hero-left px-2 lg:px-8 py-6 sm:py-[70px] lg:py-[100px] xl:py-[130px] w-full flex justify-center flex-col items-center text-center">
              <div className="hero-text sm:space-y-8 space-y-4 text-h5 font-inter text-black-100">
                ...
                {hasRichTextContent(richText) ? <div>{renderRichText(richText)}</div> : null}
              </div>
